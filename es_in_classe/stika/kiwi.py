...
class CSVTimeSeriesFile(): 
    ...
    def get_data(self):
        ...
        if self.name != 'data.csv':
            raise ExamException('il file usato non è compatibile')
    ...
    for i in range(1,13):
        ...
        for y in range(0,len(lista)):
            if lista[y][0] != first_year:
                if mese == none:              #se il mese non ha valore gli verrà messo zero
                    return 0
                if lista[y][1] == i:
...
# corpo programma          (Main) del programma
time_series_file = CSVTimeSeriesFile(name='data.csv')

time_series = time_series_file.get_data()

avg_monthly_difference=compute_avg_monthly_difference(time_series, "1949", "1951") 
#inserire intervallo (primo e ultimo anno) che si vuole considerare, solo per provare se funziona(?)
